...
async function getStatistics(month: number, year: number) {
    const data = await fetch(
        `${process.env.BASE_URL}/api/statistics?month=${month}&year=${year}`,
        {
            method: "GET",
        }
    );
...
async function getEntries(month: number, year: number) {
    const data = await fetch(
        `${process.env.BASE_URL}/api/entries?month=${month}&year=${year}`,
        {
            method: "GET",
        }
    );
                ...
                <div className="grid grid-cols-12 col-span-12 gap-6 xxl:col-span-9">
                    ...
                    <div className="col-span-12 mt-5">
                        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 2xl:grid-cols-3">
                            ...
                            >
                                <h2
                                    className={`font-bold text-base px-4 pt-4 ${
                                        loading ? "" : "-mb-4"
                                    }`}
                                >
                                    Service Days
                                ...
                                <Calendar
                                    ...
                                    enteredDays={
                                        entries
                                            ? entries.filter(entry => Number(entry.hours) !== 0).map((entry) =>
                                                  dateFromYYYYMMDD(
                                                      entry.date
                                                  ).getDate()
                                              )
                                    ...
                                    zeroedDays={
                                        entries ? entries.filter(entry => Number(entry.hours) === 0).map(entry => dateFromYYYYMMDD(entry.date).getDate()) : []
                                    }
                                />
                            </div>
                            <div
                                className="bg-white dark:bg-gray-900 shadow-lg rounded-lg 2xl:col-span-2 md:h-full h-96 md:block hidden"
                                id="chartpie"
                            >
                                <div className="flex items-center justify-center h-full overflow-hidden">
                                    <div className="-rotate-12 flex flex-col">
                                        <span className="transform xl:text-6xl text-5xl font-bold uppercase opacity-25 select-none">
                                            Under Construction
                                        </span>
                                        <span className="transform font-bold uppercase opacity-25 text-center select-none">
                                            Content coming soon
                                        </span>
                                    </div>
                                </div>
